...
# (tests/benchmark/inputs) with the DEFAULT portfolio and with --use cakeds, then
...
#
# Logs in $OUTDIR: default.csv / cakeds.csv (dense per-input), *.txt (summaries),
...
echo "=== bench sweep -> $OUTDIR  (${#FILES[@]} files, default vs cakeds, ${TO}s/input) ==="

for label in default cakeds; do
  use_args=(); [ "$label" = cakeds ] && use_args=(--use cakeds)
...
echo
echo "=== default vs cakeds (size delta) ==="
python3 tests/survey_diff.py "$OUTDIR/default.csv" "$OUTDIR/cakeds.csv"
echo
echo "logs: $OUTDIR"
