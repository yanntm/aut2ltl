...
is LTL. Dry run by default; `--prune` deletes duplicate HOA files and strips
duplicate formula lines from LTL files.

For dedup AND AP-renaming in one pass, use `survey.normalize.canon`.
...
def dropper(key: Key = default_key) -> "sweep.Op":
    ...
    a duplicate HOA file is deleted; duplicate LTL lines are stripped in place."""
    seen: set[str] = set()
...
if __name__ == "__main__":
    raise SystemExit(sweep.cli(dropper(), sys.argv[1:], verb="drop"))
