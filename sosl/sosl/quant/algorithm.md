...
## 11. The Markov product (`mc.py`, `product.py`)

Paper Thm 3.5. The source is no longer i.i.d. but a finite **chain** `M`
whose states carry one letter each (Moore): the word of a run
`q₀ q₁ q₂ …` is `ℓ(q₀)·ℓ(q₁)·ℓ(q₂)⋯`, the initial state's letter
included. That is PRISM/Storm path semantics, and it is what makes the
`.mc` file and our answer the *same* question ("same file, same
number"); dropping the first letter would answer a different one.

`.mc` is a restricted PRISM-language subset — one `dtmc`, one module,
one state variable, exactly one guarded command per state, rational
probability literals, and one `label` per letter of the alphabet, the
labels partitioning the states. The reader is strict: it validates and
raises, never repairs, and the label names are the letters' canonical
cube renderings, so alphabet equality with a paired `.sos` is *checked*,
not assumed.

The **product** is the reachable part of `states(M) × 𝒞` from
`(q₀, λ(ℓ(q₀)))`, stepping `(q, c) —P(q,q')→ (q', c·λ(ℓ(q')))`: the class
component is the fold of the word read so far. Its bottom SCCs are the
absorbing events, and their absorption probabilities solve §4's transient
system unchanged (`_solve`, one column per bottom SCC).

The generic verdict of a product bottom SCC `B` is §3 with the ambient
semigroup replaced by the one the chain actually generates. Let `q̂` be
the least `M`-state in `B` (the projection of `B` to `M` is a bottom SCC
of `M`, so `q̂` recurs a.s.). Cutting the word at the visits to `q̂` gives
a stem into `(q̂, c) ∈ B` followed by infinitely many `M`-cycles at `q̂`,
whose folds form the **cycle semigroup**

```
T := { fold(ℓ(r₁)⋯ℓ(r_m)) : q̂ = r₀ → r₁ → ⋯ → r_m = q̂, m ≥ 1 }
```

(each step reads the label of the state *entered*, so `ℓ(q̂)` closes one
cycle and opens the next). `T` is closed under product — concatenating
two cycles at `q̂` is a cycle at `q̂` — hence a subsemigroup of `S`, and
it is obtained by one BFS over `(M-state, class)` pairs from `(q̂, [ε])`,
collecting the class at every return to `q̂`; the BFS ranges over all
walks, so its collection is already the closure, no fixpoint iteration
needed.

`K_T` := the kernel of `T` — the unique sink SCC of the two-sided Cayley
graph on `T` generated by `T` itself (§2's routine, generator set `T`
instead of `λ(Σ)`), `k := idem(t)` for the least-keyed `t ∈ K_T`. Then

```
θ_B := Val(c, k),    c the shortlex-least class with (q̂, c) ∈ B
Pr_M(L) = Σ_B θ_B · Pr[absorption in B].
```

The Bernoulli case is the special case `M = ` one state per letter with
`P(q_a → q_b) = p(b)`: `T` is then all of the letter-generated `S`, `K_T`
is `K`, and Σ_a p(a)·Pr_{M_a}(L) telescopes to `μ_p(L)` — the embedding
law the gate checks byte-exact against M1, `|Σ|` times per language.

`PARANOID` (the `theta.py` flag, shared) re-derives every `θ_B` from a
second base state of `B` when it spans one, and from a second kernel
idempotent — the Lemma 3.3 invariances, now over `T`.

The product-side **Route A oracle** (`routea.route_a_chain`) is the same
absorption system over `states(M) × states(A)` for the paired
deterministic EL automaton `A`, starting at `(q₀, δ(a₀, ℓ(q₀)))`: in a
bottom SCC every product edge recurs a.s., so the run's limit mark set is
the union of the marks on the SCC's edges and the EL condition on it is
the SCC's verdict. It shares nothing with the invariant side but the
solver.
