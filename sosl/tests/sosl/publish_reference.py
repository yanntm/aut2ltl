...
Copies the raw sweep CSVs, the analyzer summaries, and the gate report. It does
not compute anything — run the analyzers/gates first; a missing source is a
warning (that output was not produced). Idempotent: re-running overwrites the
destination, so a drop is immutable only by convention (commit it, then leave it).
"""
...
FLAT_CANON: List[Tuple[str, str]] = [
    ("census/results.csv", "census_results.csv"),   # raw sweep, both legs (E1+E2)
    ...
    ("census_e3/results.csv", "e3_results.csv"),     # E3 ROLL raw
