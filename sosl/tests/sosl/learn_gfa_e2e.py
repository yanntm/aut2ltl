...
def gfa_invariant() -> Invariant:
    # Canonical GF a under the fresh-identity convention (spec §1.1): 3 classes --
    # 0 = [eps] (the fresh adjoined identity), 1 = [!a] (a no-a letter is neutral
    # but stays its own class, never merged into [eps]), 2 = [a] (contains an a).
    ab = Alphabet.of(["a"])
    return Invariant(
        alphabet=ab,
        keys=(EMPTY, (0,), (1,)),
        letter_class=(1, 2),   # !a (mask 0) -> [!a], a (mask 1) -> [a]
