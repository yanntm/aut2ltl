...
def _summary_rung(rows: List[Dict], corpus: Path) -> str:
    ...
    L.append("")
    L.append("| aps | cases | agree | recurrence share | persistence share |")
    L.append("|--:|--:|--:|--:|--:|")
    for aps in sorted({int(r["aps"]) for r in scored}):
        sub = [r for r in scored if int(r["aps"]) == aps]
        ag = sum(1 for r in sub if r["agree"] == "yes")
        rec = sum(1 for r in sub if str(r["rec_ours"]) == "1")
        per = sum(1 for r in sub if str(r["per_ours"]) == "1")
        L.append(f"| {aps} | {len(sub)} | {ag} | {rec} | {per} |")
    if not disagree:
...
def run_fixture(argv: List[str]) -> int:
    ...
    lines (encoding and canonize already vetted by construction) — never
    patched here. Emits the generated summary `logs/gt2_fixture.md`
    (census + predictions as markdown tables); promotion to
    `reference/giventhat/` is a manual `cp`."""
    ...
    def expect(label: str, got, want) -> None:
        if got == want:
            emit(f"| {label} | `{want}` | `{got}` | ok |")
        else:
            escal.append(f"{label}: got {got}, paper says {want}")
            emit(f"| {label} | `{want}` | `{got}` | **E1** |")
    ...
    emit(f"git: {_git_rev()}")
    emit("seed: deterministic (no sampling)")
    emit("corpus: n/a — built from the §4.6 formulas by "
         "`tests.giventhat.fixtures.build_gt2` (ltl2tgba + canonize)")
    emit()
    emit("# GT2 — the paper §4.6 worked example (generated by "
         "`ladder_gate --fixture`)")
    emit()
    emit("`¬φ = F(a∧c) ∨ (GFb ∧ GF¬b)`, `K = FGb ∧ Gc`, `AP = {a,b,c}`.")
    emit()
    emit("## Machine census of I(¬φ) — the count §4.6's arithmetic must reproduce")
    emit()
    emit("| class | key | idem | loops | accepting loops |")
    emit("|--:|---|---|---|---|")
    np = inv["negphi"]
    t_np = Table.of(np)
    for c in range(np.n):
        ...
        emit(f"| {c} | `{render_w(np.keys[c])}` "
             f"| {'yes' if np.mult[c][c] == c else ''} | {loops} | {acc} |")
    emit()
    emit(f"{np.n} classes, {len(t_np.linked)} linked pairs, "
         f"accept = {sorted(np.accept)}.")
    emit()
    emit("## §4.6 predictions vs the machine")
    emit()
    emit("| item | paper | machine | verdict |")
    emit("|---|---|---|---|")
    ...
    emit()
    emit("## Gates 2-4 on the fixture pair")
    emit()
    row = check_ladder_pair(inv["negphi"], inv["k"], None, None, _SEED)
    emit(f"`{row}` (band bits = {row['bits']}: the brute oracle is out of "
         f"reach here by design, `skip-bits`).")
    emit()
    if escal:
        emit("## E1 escalation (to the report, verbatim)")
        emit()
        for line in escal:
            emit(f"- {line}")
        emit()
    _LOGS.mkdir(exist_ok=True)
    (_LOGS / "gt2_fixture.md").write_text("\n".join(L) + "\n")
    print(f"wrote {_LOGS / 'gt2_fixture.md'} (promotion to reference/ is a "
          f"manual cp)")
    if escal:
        return 1
    print("SUCCESS")
