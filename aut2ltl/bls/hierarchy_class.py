...
def bls(options: Optional[Options] = None) -> Translator:
    ...
    over the Krohn-Rhodes holonomy decomposition (`as_translator`), fronted by
    `cascade_gate` (the cached LTL-definability screen, run first): a formula on
    an aperiodic (star-free) language, a PROBABLY_NOT_LTL decline otherwise.
    ...
    return cascade_gate(as_translator(make_hierarchy_class(options)))
