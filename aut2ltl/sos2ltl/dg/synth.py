...
from aut2ltl.sos2ltl.dg.compress import NodeData, compress
...
def synthesize(inv: "Invariant", max_nodes: int = 2000,
               max_arena: int = 200000) -> Tuple[Ast, int, int]:
    """The root call and the unwinding: a formula for the invariant's
    language, pure-future over `Σ^ω`. The invariant must be aperiodic
    (caller-checked; a group fires the divisor's strict-decrease assert at
    the latest). Returns `(arena, formula id, recursion node count)`."""
    frame: Frame = root_frame(inv)
    target: Target = root_target(inv, frame)
    ast = Ast()
