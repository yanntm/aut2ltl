...
proposition/lemma numbers of that text). The input is the invariant
`𝓘(L) = (𝒞, λ, M, P)` — the syntactic ω-semigroup in its canonical `.sos`
form (`sosl.sos.Invariant`): classes with shortlex keys, letter map, class
multiplication table, accepting linked pairs. The remaining open points are
collected in layer 11; the module map and build order are layer 14.

## 0 — Why this exists

The aperiodicity read-off on the invariant decides LTL-definability; on the
negative side the certificate extraction (`../witness/algorithm.md`) hands
out a replayable refutation. This module makes the positive verdict
constructive: from the aperiodic invariant it **synthesizes a defining LTL
formula**, implementing [DG] Proposition 8.1 (*AP ⊆ LTL*, by local-divisor
induction). Three properties no other engine in the portfolio has at once:
...
canonical one. The `.sos` reference construction and the learner do
(`sosl`). This module is the cash-out.
...
- **Our instance.** `h` is the syntactic morphism onto `M = S(L)₊¹` — the
  invariant's class algebra, fresh identity adjoined. It recognizes `L`
  [SωS26], and the LTL verdict says it is aperiodic. Both tables the
  synthesis reads arrive materialized in the invariant:
  - the **class multiplication table** `mult : S¹ × S¹ → S¹`;
  - the **accepting-pair verdict** `P(s, e)` = acceptance of `u·z^ω` with
    `[u] = s`, `[z] = e`: membership of `(s, e)` in `accept`, defined on
    the linked pairs. `P` is the membership evaluator for
    ultimately-periodic words presented algebraically; every "is this in
    `L`?" question below bottoms out here.
...
- **Class identity.** The invariant's class ids are already canonical:
  ascending id order is shortlex order of the class keys (the `.sos`
  normal form). All enumerations (`T₁`, `T₂`, the `K` unions, class sets)
  iterate in this id order.
...
1. **The entry toll (upstream):** materializing `𝓘(L)` — the reference
   construction's enriched-monoid closure, or the learner's queries —
   capped into a decline upstream of this module. `n` is the *minimum
   over all presentations*: the syntactic quotient is the smallest base
   every later cost is measured in.
...
- **[DG] Diekert–Gastin 2008, "First-order definable languages"**
  ...
  `L_{c,A}` device), §8 (local divisors, Prop 8.1, Lemmas 8.2–8.4). Its
  *proof* becomes the program here.
...
- **O6 — wiring.** Whether this surfaces as a `Translator` on its own, or
  only as the baseline (E4b) and scoped fallback of the transcription
  engine, is the assembly's concern — out of scope here.
...
One module per role: each owns one algorithmic step and one data shape,
consumes value objects from above, and is exercisable alone. The driver
owns **no algorithm** — sequencing, the pivot rule, memoization and caps
only.

```
        𝓘(L)  (sosl.sos.Invariant — the canonical .sos value)
                          │
                     frame.py           the recursion node's value: letters,
                          │             images, monoid, ω-universe (pairs +
                          │             conjugacy), Target; the root reads
                          │             the invariant       (layers 1, 2, 6)
        ┌─────────────────┼─────────────────┐
   ...
   strict-decrease   T₂, g, X_{n,m},   PE-unwind, Spot-syntax render
...
- **`frame.py`** — the recursion node's value `Frame`: abstract letters,
  ...
  `root_frame`/`root_target` read the `Invariant` — the root asserts the
  fresh identity stays outside the letter-generated subsemigroup, and the
  target asserts `P`-constancy per conjugacy class, the query-independence
  lemma checked on ground data.
...
**Checks** — layers 12–13 are the precomputed expected values (the walks
*are* the unit fixtures): the divisor gives `6 → {1, 5}` on the flagship
and `3 → {1}` on fairness, with the strict-decrease assert firing on any
group-bearing input; the `T₁`/`T₂`/`g`/`X_{n,m}`/`K` numbers of both walks
are compared digit for digit; the end-to-end probe asserts
Spot-equivalence of the flagship synthesis to `GF(a ∧ Xa)`; and the
layer-8 canonicity claim — two presentations of one language, one
hash-consed DAG — is one diff of arenas.
