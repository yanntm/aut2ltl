...
### 9.6 D provenance — one report paragraph (theory order, 2026-07-12)

The corpus pairs each `.sos` with a deterministic HOA `D`, and paper §8
says `D` is the §7.3 exit's output. State in the report, from the
corpus pipeline's own docs/headers (no code archaeology): (i) is `D`
produced by the §7.3 Cayley exit, or independently (e.g. Spot
determinization of the source automaton)? (ii) is `L(D) = L` checked
per row at corpus build time, and by what? If the paper sentence is
wrong, say so in the report — theory fixes the paper. No script, no
deliverable beyond the paragraph.
