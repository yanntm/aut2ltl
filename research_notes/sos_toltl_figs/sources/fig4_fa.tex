...
\begin{scope}[scale=1.0]
  ...
  \node[cls] (c1) at (0.00,-3.20) {$[\lnot a]$\\[-1pt]\textcolor{order}{\tiny 1}};
  \node[committed] (c2) at (0.00,-6.40) {$[a]$\\[-1pt]\textcolor{order}{\tiny 2}};
  \draw[eb] (c0) to node[elab] {\texttt{!a}} (c1);
  \draw[ea] (c0) to[bend left=62] node[elab] {\texttt{a}} (c2);
  \draw[eb] (c1) edge[loop left,looseness=6] node[elab] {\texttt{!a}} (c1);
  \draw[ea] (c1) to node[elab] {\texttt{a}} (c2);
  \draw[eall] (c2) edge[loop below,looseness=6] node[elab] {\texttt{!a}, \texttt{a}} (c2);
  ...
  \begin{scope}[on background layer]
    \node[layerbox, fit=(c1), inner sep=6pt] (L1) {};
  \end{scope}
  \node[layertag, anchor=west, xshift=26.0mm] at (L1.east) {\texttt{!a} $\mapsto$ reset(1)\\\texttt{a}: exit\\(A) at $k=1$\\all-rejecting: $\mathrm{STAY}^\infty = \bot$};
  \begin{scope}[on background layer]
    \node[frozenbox, fit=(c2), inner sep=6pt] (L2) {};
  \end{scope}
  \node[layertag, anchor=west, xshift=6.0mm] at (L2.east) {\textit{frozen}: every letter neutral\\all-accepting};
  \begin{scope}[on background layer]
    \draw[rorder] (L0) -- (L1);
  \end{scope}
  \begin{scope}[on background layer]
