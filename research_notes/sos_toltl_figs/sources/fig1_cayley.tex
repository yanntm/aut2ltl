...
\begin{scope}[scale=1.0]
  ...
  \end{scope}
  \node[layertag, anchor=east, xshift=-14.0mm] at (L1.west) {\texttt{!a} $\mapsto$ reset(1)\\\texttt{a} $\mapsto$ reset(3), exit@3\\(A) at $k=1$\\all-rejecting: $\mathrm{STAY}^\infty = \bot$};
  \begin{scope}[on background layer]
    \node[layerbox, fit=(c2) (c4), inner sep=6pt] (L2) {};
  \end{scope}
  \node[layertag, anchor=west, xshift=14.0mm] at (L2.east) {\texttt{!a} $\mapsto$ reset(4)\\\texttt{a} $\mapsto$ reset(2), exit@2\\(A) at $k=1$\\all-rejecting: $\mathrm{STAY}^\infty = \bot$};
  \begin{scope}[on background layer]
