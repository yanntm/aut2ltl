...
*Semantics.* An ω-word `w = x₁x₂⋯ ∈ Σ^ω` traces, from any state `q`, a
unique **run**: the state sequence `p₀ = q`, `p_i = δ(p_{i-1}, x_i)` —
matching `δ` at every step, totality and determinism leaving exactly one.
`Q` is finite, so the states a run visits infinitely often are mutually
reachable: the run eventually enters one strongly connected component (SCC)
of `D` and never leaves it. The run **collects** `mk(p_{i-1}, x_i)` at step
`i`, and `mk^∞(q, w) ⊆ F` is the set of marks it collects infinitely often —
the marks of the transitions used infinitely often, all inside that SCC. A
set of marks `N ⊆ F` **satisfies** a condition, `N ⊨ Acc`, by induction:
...
The marking extends to finite runs — `mk(q, ε) := ∅`,
`mk(q, u·x) := mk(q, u) ∪ mk(δ(q, u), x)` — the marks collected reading `u`
from `q`; a finite prefix collects finitely often, so
...
**On a lasso the semantics is finite.** Reading `u·v^ω` from `q`, the loop
iterates: `p₀ = δ(q, u)`, `p_{i+1} = δ(p_i, v)` — a sequence that closes a
cycle in an SCC of `D` — and `mk^∞(q, u·v^ω)` is the union of the
`mk(p_i, v)` around that cycle. The SCC does not decide: two lassos settling
...
sits at `p_{i+1} = p_i·⟨v⟩` at the block boundaries, collecting
`mk(p_i, v) = mk(p_i, ⟨v⟩)` in between: the boundary sequence closes the
cycle of the iteration, the marks around that cycle recur, and no other mark
does. ∎
