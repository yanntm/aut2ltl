...
> `u, u' ∈ Σ⁺`. If `𝒮_D(u) = 𝒮_D(u')` then `u ≈_L u'`. Consequently the syntactic
...
extension `d` ranges over all of `EM(D)`, identity included — `d = ⟨ε⟩`
tests the bare loop `c` itself, and `c·d` is always the image of a nonempty
word.
...
>   `𝒮_D(u)`, is the **quotient stamp**: the composition of `𝒮_D` with the
>   projection — surjective onto a finite semigroup because the lasso
>   equivalence is a two-sided congruence (rotation on runs, Lemma 4.8) —
>   with letter map sending `x` to the class of `𝒮_D(x)`, and the fresh
...
right element — `EM(D)` is letter-generated — and by rotation on runs
