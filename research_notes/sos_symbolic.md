...
lassos by stem length, then loop length, then lexicographically; the
selection must follow the layers, not the variable order. Project
`Bad` onto the small set `B = { (q₁, q₂) : ∃d }`; the least closure
layer `i` whose `(ι₁, ι₂)`-slot projection meets `B` is the stem
length, and backward preimage chaining through layers `i−1, …, 0`
choosing the least letter (Phase 6's mechanism) yields the stem `u`
and its target pair; the least layer `j` holding a `d` with
`Bad(q₁, q₂, d)` for a layer-`i`-reachable pair is the loop length,
the same chaining the loop `v`. Minimality among *all* ultimately
periodic separating words: any separating `(u′, v′)` folds to a bad
`(c′, d′)` whose layers bound `|u′|` and `|v′|` from below, and the
chained extraction is lex-least within its layer ⟨TBD: state as a
proposition; short proof over the layer invariant, including the lex
tie-break coupling stem and loop⟩. Every decision procedure of this
