# SoS Symbolic Engine — Completed Experiments (frozen archive)

The closed half of the family's ledger: completed components
(C1–C7, C9 `slot_perm`, C10 §6.1–§6.2), closed experiments (E0, E1,
E2's first points), their gates, findings **F1–F26** and Theory
responses — moved here verbatim from `sos_symbolic_report.md` at
retirement, plus the spec text they answered (end of file). This file
is frozen: entries are never edited, only referenced. The paper's
measured claims (`sos_symbolic.md` §8, RQ1–RQ5) trace here; data
lives in the tracked artefacts (`tests/sos_sdd/reference/*.csv`,
streams under `tests/sos_sdd/logs/`). The live documents are
`sos_symbolic_spec.md` (open work) and `sos_symbolic_report.md`
(active ledger, findings F27 onward).
...
- **F11 — the residual partition is the gfp, and the seed alone is
  ...
  gfp (`rounds=1`).
- **Recorded gap (closed by F21):** the spec's cross-check against the
  explicit tool's residual classes (`sosl` rides Spot's `language_map`
  on HOA inputs) waits on E1's HOA→digest bridge; the gate's ground
  truth is the same gfp computed by a deliberately different explicit
  algorithm (cycle-walk verdicts + lockstep refinement).
...
- **F22 — the §4.2 covariates, measured (and |EM¹| corroborated a
  ...
  **all 6102 instances** — an independent corroboration of the model
  ...
  adjustment before the paper cites the 62 %.⟩ (Blessed — see the F22
  response below.)

E1's scope is fully closed, including its paper reading (F23).
...
**Recorded deviation (user-settled 2026-07-12):** spec C10's alignment
bullet mandated "the aligned π-map assembled per block (Prop 6.1)"
with the assertion "Comp is never applied on the aligned space". The
engine's rendering never builds the abstract monolithic Comp anywhere
— Phase 2 is per-slot brick case splits (F8) — so on the sync-product
slot model every hom application is additive by construction and the
assertion is satisfied structurally, not by a dedicated assembly.
Alignment is therefore the ordinary build over the sync-product model;
Prop 6.1's per-block assembly is demoted to a measured optimization
(revisit if E9 prices aligned re-pairing as dominant), and the
proposition is validated end to end through readings and byte gates.
*Resolution: folded into the spec's C10 alignment bullet — the settled
rendering is now normative there.*

---

## Retired spec text (the definitions these findings answered)

### E0 — sanity on the worked examples (gated M1)
...
*(Outcome: all confirmed — F1, F2, F14.)*
...
(F22 response): per-`(slot, dst)` families, one-mark closure
...
correlate analysis is done (F23): per-slot sharing dominates,
...
2k-relation encoding; C5 (profiles/residuals, oracle-free) and C6
(congruence, rotation lemma structural) element-exact on seven cases;
C7 conformance byte-gate green (identity convention F15 the one
divergence found); E1 closed at corpus scale (6102 byte-identical,
120 TIME_BUDGET, covariates + read-offs F17–F23); C9 `slot_perm` live
(F24); C10 §6.1 membership (F25) and §6.2 Boolean algebra (F26) done
with commutation gates. The full bullet-by-bullet narrative lives in
this file's findings above and in git history of
`sos_symbolic_spec.md`.
