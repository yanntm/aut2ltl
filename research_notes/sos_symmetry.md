...
- **The group spectrum** (§6): read off the maximal subgroups the
  ...
  the counting content of `L` confined to the gap `L♯ ∖ L♭` — proved
  in full: the reflection is the least aperiodic congruence and the
  collapse-and-close iteration computes it exactly (Lemma 6.2a), the
  pair sort is governed by a lifting/saturation lemma (Lemma 6.2b),
  and the kernel is the complement-dual of the hull.
...
Two linked pairs are **conjugate**, written `(s, e) ≃ (s', e')`, if
there are `x, y` with `e = xy`, `e' = yx`, `s' = sx` [PP04]. This is
an equivalence (symmetry: take `(x', y') := (y, x)`, using
`s = se = sxy = s'y`; reflexivity: `(x, y) := (e, e)`), and
conjugate pairs receive the same verdict in any recognition:
`s'·e'^ω = sx·(yx)^ω = s·(xy)^ω = s·e^ω`. Every linked pair is the
induced pair of a lasso (for `u ∈ s`, `v ∈ e`, the induced pair of
`u·v^ω` is `(s·e^π, e^π) = (s, e)`, writing `x^π` for the unique
idempotent power of `x`), and the induced pairs of two lassos
presenting the *same* ω-word are conjugate: lasso presentations are
related by shift (`(u, v) → (uv, v)`), power (`(u, v) → (u, v^k)`) —
both of which fix the induced pair, since `(x^k)^π = x^π` — and
rotation (`(u, wz) → (uw, zw)`), which conjugates it via
`x := (wz)^π·w`, `y := z·(wz)^{π−1}`.
...
**Lemma 6.2a (the aperiodic reflection, computed).** Among the
congruences of `𝒞` with aperiodic quotient there is a least one,
`θ_ap` (the *aperiodic reflection* of the table), and the
collapse-and-close iteration computes it exactly: starting from
`𝒞₀ = 𝒞`, let `ρ_{i+1}` be the congruence of `𝒞_i` generated by
`{ (x, e_H) : x ∈ H, H a nontrivial maximal subgroup of 𝒞_i with
idempotent e_H }` and `𝒞_{i+1} = 𝒞_i/ρ_{i+1}`; the iteration
terminates, and the kernel of the composite projection is `θ_ap`.

*Proof.* Termination: each nontrivial round strictly shrinks a
finite semigroup. The terminal quotient has no nontrivial maximal
subgroup, hence no nontrivial subgroup at all — a subgroup `G` with
identity `e` sits inside the `H`-class of `e` (each `g ∈ G` has
`g = eg = ge` and `e = g·g⁻¹ = g⁻¹·g`, so `g H e`), which is the
maximal subgroup at `e` — so the terminal quotient is aperiodic
(§2.6), i.e. the composite kernel `ρ*` is an aperiodic congruence.
Leastness: let `θ` be any congruence with `𝒞/θ` aperiodic; we show
`ker(π_i) ⊆ θ` for every stage `i` of the composite projection
`π_i : 𝒞 → 𝒞_i`, by induction. For `i = 0` this is trivial. If
`ker(π_i) ⊆ θ`, then `θ` induces a congruence `θ_i` on `𝒞_i` with
`𝒞_i/θ_i ≅ 𝒞/θ` aperiodic; the image of a maximal subgroup `H` of
`𝒞_i` under `𝒞_i → 𝒞_i/θ_i` is a subgroup of an aperiodic
semigroup, hence trivial, so `H × H ⊆ θ_i` — every generator of
`ρ_{i+1}` lies in `θ_i`, hence `ρ_{i+1} ⊆ θ_i` and
`ker(π_{i+1}) ⊆ θ`. Thus `ρ* ⊆ θ` for every aperiodic congruence
`θ`, and `ρ*` is itself aperiodic: `ρ* = θ_ap`, which in particular
exists and is unique. ∎

(Whether a single round always suffices we do not know — the
iteration count is a census datum; `EvenHead` below closes in one
round.)

Write `T := 𝒞/θ_ap` and `q : 𝒞 → T`. The ω-sort of the reflection
is governed by two facts about any quotient of the finite sort:

**Lemma 6.2b (lifting and saturation).** Let `q : 𝒞 → T` be a
surjective morphism of finite semigroups.

(i) *(lifting)* Every linked pair of `T` is the `q`-image of a
linked pair of `𝒞`.

(ii) *(the saturation law)* For `A ⊆ linked(T)`, the verdict
"`u·v^ω` accepted iff its induced pair over `T` lies in `A`" is
independent of the lasso presenting the word **iff** `A` is closed
under conjugacy. In that case `(T, q∘λ, M_T, A)` presents an
ω-regular language, with membership the usual fold-and-lookup
[SωS26].

*Proof.* (i) Given `(t, f)` linked in `T`, pick `u ∈ q⁻¹(t)`,
`v ∈ q⁻¹(f)`, and set `e := v^π`, `s := u·e`: then `q(e) = f^π = f`,
`q(s) = t·f = t`, and `(s, e)` is linked.

(ii) (⟸) Two lassos presenting the same word are related by shift,
power, and rotation moves (§2.2); the first two fix the induced
pair, rotation conjugates it, and `A` is conjugacy-closed. (⟹) If
`(t, f) ∈ A` but `(t', f') ∉ A` with `(t, f) ≃ (t', f')` via
`x̄, ȳ`, realize `t = q(λ(u))`, `x̄ = q(λ(w_x))`, `ȳ = q(λ(w_y))`
by words (`q∘λ` generates `T`); then `u·(w_x w_y)^ω` and its
rotation `u w_x·(w_y w_x)^ω` present the same ω-word with induced
pairs `(t, f)` and `(t', f')` — the verdict is ill-defined. ∎

By (ii), the pair sets of the reflection must be conjugacy-closed;
the saturations of `P` are therefore

```
Acc♯ = conj_T( q(P) )                   (hull: classes reachable from P)
Acc♭ = linked(T) ∖ conj_T( q(linked(𝒞) ∖ P) )   (kernel: classes that cannot exit P)
```

where `conj_T` is conjugacy closure over `T` — note the closure
*after* projecting: componentwise `θ_ap`-saturation of `P` alone
(`q⁻¹(q(P))`) is not enough in general, because two pairs can
become conjugate only in the quotient (Lemma 6.2b(ii) makes the
unclosed verdict ill-defined, which is why the implementation
saturates after every collapse round). Both sets are
conjugacy-closed (`Acc♭` as the complement of a closed set).

**Proposition 6.2 (LTL hull and kernel).** The tables
`(T, q∘λ, M_T, Acc♯)` and `(T, q∘λ, M_T, Acc♭)` present languages
`L♯` and `L♭` with:

(i) `L♭ ⊆ L ⊆ L♯`;

(ii) both are LTL-definable;

(iii) *(optimality)* every language containing `L` that is
presented over an aperiodic congruence quotient of `𝒞` (any `θ`
with `𝒞/θ` aperiodic, any conjugacy-closed pair set) contains
`L♯`; dually, every such language contained in `L` is contained in
`L♭` — `L♯` is the least superset and `L♭` the greatest subset of
`L` recognized through the aperiodic reflection of `L`'s own
algebra;

(iv) *(canonicity)* both are canonical in `L`: byte-equal keyed
invariants for equal input languages;

(v) *(duality)* `L♭ = ((L^c)♯)^c` — the kernel is one free
complement on each side of the hull (§2.4);

(vi) off the gap `L♯ ∖ L♭`, the three languages agree: `α ∈ L♭`
forces `α ∈ L`, and `α ∉ L♯` forces `α ∉ L` — membership in `L` is
decided by the aperiodic reflection everywhere except on the gap.

*Proof.* Both tables are well-formed by Lemma 6.2b(ii), so `L♯`,
`L♭` exist and are ω-regular; throughout, ω-regular languages are
compared on their ultimately periodic members (§2.1), and the
induced pair over `T` of a lasso is the `q`-image of its induced
pair over `𝒞`.

(i) If `u·v^ω ∈ L` its pair lies in `P`, so its `T`-pair lies in
`q(P) ⊆ Acc♯`. If `u·v^ω ∈ L♭`, its `T`-pair avoids
`conj_T(q(linked ∖ P))` ⊇ `q(linked ∖ P)`, so its `𝒞`-pair is in
`P`.

(ii) The syntactic invariant of `L♯` is a further quotient of the
presenting table (reduce only merges classes, [SωS26]), so `𝒞(L♯)`
is a homomorphic image of `T`; aperiodicity is equational
(a finite semigroup is aperiodic iff `xⁿ = x^{n+1}` for all `x`,
with `n` its size) and equations pass to homomorphic images, so
`𝒞(L♯)` is aperiodic. By §2.6, `L♯` is LTL-definable; likewise
`L♭`.

(iii) Let `L' ⊇ L` be presented by `(𝒞/θ, …, A')` with `𝒞/θ`
aperiodic and `A'` conjugacy-closed. By Lemma 6.2a, `θ_ap ⊆ θ`,
so the projection factors as `g : T → 𝒞/θ`. Take
`u·v^ω ∈ L♯`: its `T`-pair `(t, f)` is conjugate to some
`(q(s), q(e))` with `(s, e) ∈ P`. Realize `(s, e)` by a lasso
`u₀·v₀^ω` (`u₀ ∈ s`, `v₀ ∈ e`, §2.2): it lies in `L ⊆ L'`, so its
`𝒞/θ`-pair `g(q(s), q(e))` lies in `A'`. Applying `g` to a
conjugation carries conjugate pairs to conjugate pairs, so the
`𝒞/θ`-pair of `u·v^ω`, namely `g(t, f)`, is conjugate to
`g(q(s), q(e)) ∈ A'`, hence in `A'`: `u·v^ω ∈ L'`. The dual
statement follows from (v) applied to `L'^c ⊇ L^c`.

(iv) `θ_ap` is unique (Lemma 6.2a), conjugacy closure and the
saturations are defined from the table alone, and the canonical
keying of the reduce pass is canonical [SωS26].

(v) Complement is the `P`-flip within the linked pairs (§2.4):
`(L^c)`'s pair set is `linked(𝒞) ∖ P`, its hull acceptance is
`conj_T(q(linked ∖ P))`, and flipping that within `linked(T)`
gives exactly `Acc♭`.

(vi) Immediate from (i). ∎

The two remarks the proof earns: first, optimality is relative to
`L`'s own algebra — a *least LTL superset* of a non-LTL language
need not exist at all, so (iii) is the strongest canonical
statement available; second, by (v) the implementation needs only
the hull (kernel = complement ∘ hull ∘ complement, both
complements free).
...
`P = {(B, B), (C₀, B)}`, of eight linked pairs. Collapsing
`A₀ ∼ A₁` and closing to a congruence forces `C₀ ∼ C₁` and nothing
else: the reflection has **five** classes `{1, A, B, C, D}` and is
aperiodic in one round. Saturating `P` over the quotient (here the
conjugacy closure of Lemma 6.2b adds nothing — both acceptance sets
are already conjugacy-closed):

```
Acc♯ = conj(q(P)) = {(B,B), (C,B)}                  →  L♯ = a^*·b^ω   (hull: drop the parity)
Acc♭ = linked ∖ conj(q({(C₁,B),(D,B),(A₁,A₀),(A₀,A₀),(D,A₀),(D,D)}))
     = {(B,B)}                                      →  L♭ = {b^ω}     (kernel: what cannot exit `P`)
```
...
- **P5 — `EvenHead` inner data.** `Spec = {Z/2}`; the aperiodic
  reflection has 5 classes, reached in one collapse round; the
  conjugacy closure of the acceptance sets (Lemma 6.2b) is trivial
  on this specimen — `conj(q(P)) = q(P)`; the hull
