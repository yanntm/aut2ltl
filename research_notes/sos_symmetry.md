...
*Working draft — 2026-07-11 (second pass: paper skeleton). Every
unproven leap is flagged ⟨TBD⟩.*
...
*computes* them: each is a language-equivalence question, PSPACE per
candidate on automata, so tools substitute syntactic checks on the
formula that are sound, incomplete, and silent on failure. We show
...
On automata, every instance of kinds 1 and 2 is a language-equivalence
question — PSPACE-complete per candidate, and absent from the standard
toolbox: symmetry is not an invariant of a presentation, so no
automaton you hold exhibits it. Kind 3 is invisible on automata
...
- **The group spectrum** (§6): read off the maximal subgroups the
  ...
  6.4**: the aperiodic reflection of the invariant yields canonical
...
Arnold's congruence [Arn85]: `u ∼_L v` iff for all finite `x, y` and
all `β ∈ Σ^ω`, `x·u·y·β ∈ L ⟺ x·v·y·β ∈ L` and
`x·(u·y)^ω ∈ L ⟺ x·(v·y)^ω ∈ L`. The quotient is the syntactic
ω-semigroup, the minimal recognizer. [SωS26] computes and canonically
...
definable ⟺ `L` is LTL-definable ([Tho79]; survey [DG08]); [SωSX26]
implements this frontier on the invariant, with counting-family
certificates on refusal. A group *divides* a semigroup if it is a
quotient of a subsemigroup; the maximal subgroups of `𝒞` are its
group `H`-classes at idempotents. Krohn–Rhodes theory [KR65]
supplies: a simple group divides a finite semigroup iff it divides
one of its maximal subgroups ⟨TBD: cite the precise
Krohn–Rhodes-adjacent statement⟩. On finite words, solvable monoids
correspond to `FO+MOD` definability (Straubing–Thérien–Thomas;
[Str94]).
...
`[u] = s, [v] = e`, and `σ(u·v^ω) = σ(u)·σ(v)^ω ∈ σ(L) = L`.

(ii ⟺ iii): `(𝒞, λ∘σ, M, P)` is the invariant of `σ⁻¹(L)` (the
syntactic congruences correspond under the relabeling, so no
re-quotient occurs and reduce is keying only); byte equality is
language equality [SωS26, Thm 5.1], and `σ⁻¹(L) = L ⟺ σ(L) = L`.
...
groups on the `λ`-fibers and is *always* contained in `Sym(L)` — the
invariant has already quotiented these symmetries away; the semantic
...
**Example A (an inert atomic proposition — the kernel; observed in
corpus curation).** `L_A = GF a` over `AP = {a, b}`. By hand:
`𝒞 = {1, F, N}` with `F` = [contains an `a`-letter] (absorbing:
`xF = Fx = F`), `N` = [contains none] (idempotent); `λ` sends the two
`a`-minterms to `F` and the two `¬a`-minterms to `N`;
`P = {(F, F)}`. The fibers are fat, and the polarity flip `σ_b`
...
`|P| = 1` against three-plus linked pairs: no anti-symmetry, decided
by counting alone.
...
These are exactly the two clauses of Arnold's congruence (§2.3), so
`u ∼_L v`, i.e. `[u] = [v]`. ∎
...
Each rung is `|Σ|^k` table equations. Where a language enters the
...
*Proof.* Closure under `(ab ↔ ba)` is Theorem 4.2 with `u = ab`,
`v = ba`; maximality is per-pair, since the closures for different
pairs are independent conditions. ∎
...
**Upgrade to `stutterquotient`.** The leaf translator
[stutterquotient.md] searches a *determining alphabet* on a
presentation's SCC, heuristically. On the invariant the analogous
split is canonical: the don't-care letters of a layer are its neutral
letters (the `St`-structure of [ToLTL26] §4.2), and the global
padding letters are the `[c] = 1` read-off — the search becomes a
read-off, per layer, with no presentation in sight.
...
as the set of simple groups dividing `𝒞`; equivalently (§2.6, ⟨TBD:
precise Krohn–Rhodes citation [KR65]⟩) the union of the composition
factors of the maximal subgroups. `Spec` is canonical, computed by
...
- `Spec(L)` solvable (indeed within `{Z/p}`) should correspond to
  ...
  `FO+MOD` (§2.6) to the ω-setting ⟨TBD: the ω-transposition —
...
*which counting repairs it*: `EvenBlocks` has `Spec = {Z/2}` and is
LTL+`MOD₂`; a language with `Spec = {Z/3}` needs ternary counting;
a nonabelian simple divisor would place the language outside every
`FO+MOD` extension. The oracle already walks these groups — the
column is free.
...
**Proposition 6.4 (LTL hull and kernel).** The invariants
...
membership in `L` *is* the parity of the head — Proposition 6.4's
...
The cascade thread's determinacy analysis ([BLS-C26] C.4) found
exactly two mechanisms by which equal recurrence data can split a
verdict: *group cancellation* (`EvenBlocks`) and *zero absorption*
(the residual floor). The spectrum is the census-wide map of the
first; the sandwich scan (K-E7) classifies both. A language with
empty spectrum can still be hard for bounded-recurrence conditions —
absorption is aperiodic — but it is at least LTL; a language with
nonempty spectrum is not LTL at all, and §6.2 confines the damage.
Symmetry (a group) and saturation (a zero) sit at the two ends of
the `J`-order, and both are read off the same table.
...
existential projection, which the calculus prices honestly at the
...
Proposition 6.4 gives a sound two-sided LTL screen for verifying
...
symmetry check in all three is syntactic on the formula. Partial
order reduction: Peled [Pel93], Godefroid [God96]; the spec-side
conditions are stutter-invariance plus visibility approximations.
...
- **`Spec(L)`** over the 1 698 non-LTL languages: expected
  ...
  a find. Cross-tabulate with Wagner degree.
...
- **P1 — invariant sizes.** `|𝒞(L_C)| = 3` (Example C, `a·Σ^ω`),
  `|𝒞(L_A)| = 3` (Example A, `GFa` over 2 APs), `|𝒞(L_B)| = 5`
  (Example B, `GFa ∧ GFb`), `|𝒞(EvenHead)| = 7`.
...
- **P3 — the pair-count obstruction.** Example C satisfies
  `2|P| = |linked|` (2 of 4); Examples A and B are refuted for
  anti-symmetry by the count alone.
...
`sos_symmetry_report.md`. The census tooling covers all four columns
without new infrastructure beyond the stabilizer search.

## References (tags used; full list on landing)

- **[Arn85]** A. Arnold. *A syntactic congruence for rational
  ω-languages.* TCS 39 (1985).
...
- **[CEFJ96]** E. Clarke, R. Enders, T. Filkorn, S. Jha. *Exploiting
  symmetry in temporal logic model checking.* FMSD 9 (1996).
...
- **[Tho79]** W. Thomas. *Star-free regular sets of ω-sequences.*
  Inf. & Control 42 (1979).
- ⟨TBD, to fetch: Diekert–Muscholl (infinite traces);
  Straubing–Thérien–Thomas (FO+MOD); the ω-MOD analogue; folklore
  source for finite-word Theorem 3.1.⟩
