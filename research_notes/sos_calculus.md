...
where `Reach(s) := s·𝒞¹ ∩ (linked stems)` — Proposition 3.5 makes any
...
divide each other on the right — Green's relation `R`. So:
...
degree is finite, iff `m(L) = 0` — no *chain* of length 1, where by
the Carton–Perrin normal form [CP97, Thm 6] a chain of length 1 is a
linked stem `s` with two loops `e >_H f` (`ef = fe = f`, `e ≠ f`) of
*different* verdicts [Wag79; CP99, Thm 6 and Cor 7; SW08]. Two lemmas
take us from `m = 0` to stem-only verdicts. Throughout, note that if
...
**Lemma 3.8 (loops over one stem are connected).** Let `P` be
...
normalize `T`'s Rees presentation over its rows `{1, 2}` and columns
...
**Lemma 3.9 (blind verdicts are `R`-invariant).** Let `P` be saturated
...
**Theorem 3.10 (the obligation rung is hull-generated).** For an
...
(2)⟹(3): Lemma 3.8 gives stem-only; Lemma 3.9 upgrades stem-only to
...
(Proposition 3.5), and Boolean combinations of safety properties are
